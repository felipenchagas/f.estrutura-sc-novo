...
if(el_width<640){el_quality='medium';};window.setTimeout(function(){if(!YT.loaded){console.log('not loaded yet');window.setTimeout(arguments.callee,50)}else{var curplayer=video_control(video_id,video_source);}},50);};}
function video_control(uniqid,video_source){var interval;var player;var chek=0;player=new YT.Player(uniqid,{height:el_height,width:el_width,videoId:video_source,playerVars:{'autoplay':1,'rel':0,'showinfo':0,'showsearch':0,'controls':0,'loop':1,'enablejsapi':1,'theme':'dark','modestbranding':0,'wmode':'transparent','enablejsapi':1,},events:{'onReady':onPlayerReady,'onStateChange':onPlayerStateChange}});window.addEventListener('focus',function(){checkPlayer();return true;});function onPlayerReady(event){event.target.mute();player.setPlaybackQuality(el_quality);player.playVideo();}
...
function Video_resizer(){if(element.length){for(var i=element.length-1;i>=0;i--){video_source=element[i].getAttribute("data-video-source");video_id=element[i].getAttribute("data-video-id");el_width=document.getElementsByClassName("cws_Yt_video_bg")[0].offsetWidth;if(element[i].offsetHeight<((el_width/16)*9)){el_height=(document.getElementsByClassName("cws_Yt_video_bg")[0].offsetWidth/16)*9;}else{el_height=document.getElementsByClassName("cws_Yt_video_bg")[0].offsetHeight;el_width=(el_height/9)*16;}
...
jQuery(document).ready(function(){});jQuery(window).resize(function(){});jQuery(window).load(function(){onYouTubePlayerAPIReady()});